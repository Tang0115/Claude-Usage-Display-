...
- `daemon.py` — runs two independent loops that both write to `usage.json`: the main loop polls the Anthropic API every 15 seconds using your OAuth token for usage data, and Pi stats (CPU %, RAM %, CPU temp via `psutil`); a background thread polls Spotify's currently-playing endpoint (if set up) for track/album/artist/art/progress — including podcast episodes and DJ sessions — every 2 seconds while something's playing, backing off to 20 seconds while idle (and further if Spotify rate-limits the requests) — so switching songs shows up almost instantly without hammering Spotify's API. Claude usage polling is deliberately kept at 15s rather than faster, since every poll is itself a real API call that counts against your own 5h/7d rate-limit window. Automatically refreshes both the Claude and Spotify access tokens before they expire, using the refresh tokens from `~/.claude/.credentials.json` and `~/.spotify_credentials.json` respectively — no manual intervention needed. On boot, retries the initial token check until the network is available
